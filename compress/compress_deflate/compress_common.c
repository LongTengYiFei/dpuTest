 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t zlib_compatible_callback(void *param, void *config)
{
	struct compress_cfg *compress_cfg = (struct compress_cfg *)config;
...
{
	doca_error_t result;
	struct doca_argp_param *zlib_compatible;
...
doca_error_t register_lz4_stream_params(void)
{
	...
	doca_argp_param_set_description(has_block_checksum, "Flag to indicate if blocks have a checksum");
	...
	doca_argp_param_set_description(are_blocks_independent, "Flag to indicate if blocks are independent");
